...
**防作弊棘轮 (anti-gaming ratchet)**：验证只能**新增**证据，不得靠删除 / 跳过 / 注释掉 / 弱化既有测试或验收项来把节点标成"通过"。要修改任一条验收标准（含放宽测试），必须显式记录原因并按 §9 经人工确认——这是 §1「不能擅自降低用户目标」在测试层面的落地。
...
每轮**追加写、不回改**地记录：当前目标 / 方案图 / 当前节点 / 执行动作 / 工具调用 / 输出证据 / 验证结果 / 失败原因 / 返工方案 / 审查报告 / 裁决原因。这是给恢复、审查、复盘用的证据链，不是给模型看的流水账。

硬要求：新会话**仅凭该日志 + git 历史**就能重建方案图当前状态（哪些节点已验证通过、当前卡在哪、下一步做什么），不依赖上一会话残留在上下文里的记忆。

**上下文交接触发 (compaction handoff)**：当上下文接近上限、或单个节点要跨会话继续时，先把「方案图当前状态 + 已验证证据 + 下一步动作」写进交接文件（复用 `handoff` / `session-handoff` skill 作为该节点能力）再重置或结束会话——状态活在 Agent 上下文之外，而不是赌它还记得。
