...
1. **默认 Loop 原生拆解方案图。** 满足任一条件才把规划委托给 Ralph 的 `/prd -> prd.json`：
   - 工作量预计跨 1 个以上会话
   - 垂直切片 > 5 个
   - 明显跨模块 / 跨子系统的特性
2. **一个方案节点默认只调用一个能力**（如 `tdd` 或 `diagnose`）。需要叠加时，以该节点的**首要验收标准**决定主导能力，另一个降为子步骤。
3. **任何能力（含 Ralph / Skill）声称"完成"，Loop 不照单全收**，一律按 §4 用独立证据（测试 / 构建 / 真实运行 / 文件检查）复核。
